...
| 🟡 | Guesses at operator Custom Resource field names. | The corpus carries each operator's [CR options reference](https://docs.percona.com/percona-operator-for-mongodb/operator.html), so a `PerconaServerMongoDB` (or PXC/PG) manifest cites real fields. |
